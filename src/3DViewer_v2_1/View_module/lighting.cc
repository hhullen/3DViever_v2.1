...
Lighting::Lighting(QFont &font, QWidget *parent)
    : QWidget(parent), ui_(new Ui::Lighting) {
  this->setFont(font);
  ui_->setupUi(this);
  ...
  connect(ui_->palette_light, &QPushButton::clicked, this,
          &Lighting::GetNewColor);
  connect(ui_->cb_switcher, &QComboBox::currentIndexChanged, this,
          &Lighting::SwitchState);
  connect(ui_->d_spin_box_power, &QDoubleSpinBox::valueChanged, this,
          &Lighting::DataUpdatedSignal);
...
QColor Lighting::get_color() { return color_; }

bool Lighting::IsTurnedOn() { return ui_->d_spin_box_power->isEnabled(); }
