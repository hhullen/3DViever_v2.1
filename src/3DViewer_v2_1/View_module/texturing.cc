...
Texturing::Texturing(QFont &font, QWidget *parent) :
    ...
    connect(ui_->btn_open, &QPushButton::clicked, this, &Texturing::OpenMediaFolder);
}
...
void Texturing::SaveUVMap() {
    ...
    QDateTime date_time;
    QString filename = map_path_ + "/" + "map_" + date_time.currentDateTime().toString("yyyy_MM_dd_hh_mm_ss") + ".bmp";
...
QLineF Texturing::MakeLine(size_t i, size_t shif1, size_t shift2) {
    QPointF point1((*data_)[i - shif1] * texture_.width(), (*data_)[i - shif1 + 1] * texture_.width());
    QPointF point2((*data_)[i - shift2] * texture_.width(), (*data_)[i - shift2 + 1] * texture_.width());
