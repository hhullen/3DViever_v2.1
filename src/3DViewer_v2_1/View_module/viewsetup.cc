...
QColor ViewSetup::get_polygon_color() { return polygon_color_; }
...
void ViewSetup::ChoosePolygonColorSlot() {
  polygon_color_ =
      GetNewColor(ui_->palette_polygon, ui_->label_polygon_rgb, polygon_color_);
  emit DataUpdatedSignal();
}

void ViewSetup::ResetSlot() {
  PanelFuncs::SetColor(QColor(0, 0, 0, 255), &vertex_color_,
                       ui_->palette_vertex);
  PanelFuncs::SetColor(QColor(255, 255, 255, 255), &background_color_,
                       ui_->palette_background);
  PanelFuncs::SetColor(QColor(255, 23, 190, 255), &edge_color_,
                       ui_->palette_edge);
  PanelFuncs::SetColor(QColor(41, 26, 183, 255), &polygon_color_,
                       ui_->palette_polygon);
...
void ViewSetup::SaveSettings() {
  ...
  settings_->setValue("drawing_type", ui_->cb_drawing_type->currentIndex());
...
void ViewSetup::UploadSettings() {
  ...
  ui_->cb_drawing_type->setCurrentIndex(
      settings_->value("drawing_type").toInt());
  ManageDrawingDependenciesSlot(settings_->value("drawing_type").toInt());

  temp = settings_->value("background_color");
  PanelFuncs::SetColor(temp.value<QColor>(), &background_color_,
                       ui_->palette_background);
  PanelFuncs::ShowChosenColorInfo(ui_->label_background_rgb, background_color_);
  temp = settings_->value("vertexes_color");
  PanelFuncs::SetColor(temp.value<QColor>(), &vertex_color_,
                       ui_->palette_vertex);
  PanelFuncs::ShowChosenColorInfo(ui_->label_vertex_rgb, vertex_color_);
  ...
  PanelFuncs::SetColor(temp.value<QColor>(), &polygon_color_,
                       ui_->palette_polygon);
